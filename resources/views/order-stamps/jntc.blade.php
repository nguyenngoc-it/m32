...
?>
    <table class="tbl_wrapper" border="1" cellspacing="0" cellpadding="0">
        ...
        <tr>
            ...
            <td class="text-center">{{ $order->receiverWard->label ?? '' }} - {{ $order->receiverDistrict->label ?? '' }}</td>
        </tr>
        <tr class="fz-23 font-bold text-center">
            <td>{{ $shippingLocation->meta_data['sortingcode'] ?? '--' }}</td>
            <td>{{ $shippingLocation->meta_data['sortingNo'] ?? '--' }}</td>
        </tr>
        <tr>
