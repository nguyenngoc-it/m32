...
class NIJAVAPShippingPartner extends AbstractShippingPartner implements OrderStampRenderable
{
    ...
    {
        $this->logger   = LogService::logger('nijavap');
...
//            'requested_tracking_number' => $order->ref,
            ...
            'from' => [
                ...
                'email' => $this->settings['sender']['email'],
                'address' => [
        ...
        if (app()->environment() == 'local') {
            $dataReturn['to']['address']['postcode']  = $order->receiver_postal_code;
        }
    ...
    {
        $token = Cache::get('token_nijavap_' . $this->settings['client_id']);
        if ($token) {
            return $token;
        }
        $jsonParams = [
        ...
        if ($success) {
            ...
            if ($token) {
                Cache::put('token_nijavap_' . $this->settings['client_id'], $token, 60 * 60 * 100);
                return $token;
            }
        }
    ...
    {
        return [ShippingPartner::CARRIER_NIJAVAP];
    }
    ...
    public function createOrder(Order $order)
    {
        ...
        $res = $this->sendRequest(function () use ($jsonParams) {
            return $this->http->post($this->apiUrl . '4.1/orders', [
                'json' => $jsonParams
            ]);
        });
        ...
        if ($success == true) {
            ...
            if ($this->errorTokenInvalid($res->getData())) {
                Cache::forget('token_nijavap_' . $this->settings['client_id']);
            }
            throw new ShippingPartnerException("NIJAVAP - Create Order Error");
        }
        return null;
    }
    ...
    {
        return view('order-stamps/ninjavanp', ['orders' => $orders]);
    }
