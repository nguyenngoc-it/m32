...
use Modules\NIJAVAP\Services\NIJAVAPShippingPartner;
use Modules\ShippingPartner\Models\ShippingPartner;
use Modules\ShippingPartner\Models\ShippingPartnerLocation;

class MapLocationsCommand extends Command
{
    protected $signature = 'NIJAVAP:map-locations';

    protected $description = 'Mapping NIJAVAP locations with system locations';
    ...
    public function handle()
    {
        ...
        $NIJAVAPShippingPartner              = new NIJAVAPShippingPartner($shippingPartner->settings);
        $shippingPartnerLocationProvinces = ShippingPartnerLocation::query()->where([
            'partner_code' => ShippingPartner::PARTNER_NIJAVAP,
        ...
        foreach ($shippingPartnerLocationProvinces as $shippingPartnerLocationProvince) {
            ...
            foreach ($shippingPartnerLocationDistricts as $shippingPartnerLocationDistrict) {
                $jntDistricts = $NIJAVAPShippingPartner->getLocations($shippingPartnerLocationProvince->name, $shippingPartnerLocationDistrict->name);
