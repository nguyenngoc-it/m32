...
use Modules\JNTP\Jobs\SyncQueryTrackingJob;
use Modules\Order\Models\Order;
use Modules\ShippingPartner\Models\ShippingPartner;

class QueryTrackingCommand extends Command
{
    protected $signature = 'jntp:query-trackings';

    protected $description = 'Lấy danh sách đơn hàng của JNTP để cập nhật thông tin trạng thái';

    public function handle()
    {
        $shippingPartners = ShippingPartner::query()->where('partner_code', ShippingPartner::CARRIER_JNTP)->get();
        ...
        foreach ($shippingPartners as $shippingPartner) {
            $orders      = $shippingPartner->orders()
                ...
                ->where('created_at', '>', Carbon::now()->subMonth())->orderBy('orders.id');
