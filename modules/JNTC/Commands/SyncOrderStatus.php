...
namespace Modules\JNTC\Commands;
...
class SyncOrderStatus
{
    ...
    {
        $this->logger = LogService::logger('jntc-hook');
        ...
        if (!$orderStatus = Service::jntc()->mapStatus($status)) {
            $this->logger->error('WEBHOOK.CANT_MAP_STATUS');
            throw new ShippingPartnerException("JNTC.WEBHOOK.CANT_MAP_STATUS: " . json_encode([
                    'order' => $orderCode,
    ...
    {
        $query = Order::query()->where('shipping_partner_code', ShippingPartner::PARTNER_JNTC);
        if (!empty($code)) {
