...
namespace Modules\SAPI\Commands;
...
class SyncOrderStatus
{
    ...
    public function handle()
    {
        ...
        if (!$order = $this->findOrder($referenceNo, $trackingReferenceNo)) {
            ...
            return null;
//            throw new ShippingPartnerException("SAPI.WEBHOOK.ORDER_NOT_FOUND: {$referenceNo}");
        }

        $order->update(['original_status' => $status]);

        if (!$orderStatus = Service::sapi()->mapStatus($status)) {
            $this->logger->error('WEBHOOK.CANT_MAP_STATUS');
            throw new ShippingPartnerException("SAPI.WEBHOOK.CANT_MAP_STATUS: " . json_encode([
        ...
        if (!$order->canChangeStatus($orderStatus)) {
            ...
            return null;
//            throw new ShippingPartnerException("SAPI.WEBHOOK.CANT_CHANGE_STATUS: " . json_encode($errorPayload));
        }
        ...
        try {
            ...
            throw new ShippingPartnerException("SAPI.WEBHOOK.CHANGE_STATUS_FAILED: " . json_encode($errorPayload));
        }
    }
    ...
    {
        return Order::query()->where('shipping_partner_code', ShippingPartner::PARTNER_SAPI)
