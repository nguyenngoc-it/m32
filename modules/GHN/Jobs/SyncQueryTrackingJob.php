...
namespace Modules\GHN\Jobs;

use App\Base\Job;
use Gobiz\Workflow\WorkflowException;
use Modules\GHN\Services\GHNShippingPartner;
...
class SyncQueryTrackingJob extends Job
{
    ...
    public function handle()
    {
        ...
        $ghn    = new GHNShippingPartner($this->shippingPartner->settings, config('services.ghn.api_url'));

        foreach ($this->trackingNos as $trackingNo) {
            $trackings = $ghn->getTrackings([$trackingNo]);
