...
namespace Modules\GHN\Commands;
...
class SyncOrderStatus
{
    ...
    public function __construct(array $input, User $user)
    {
        $this->input = $input;
        $this->user = $user;
        $this->logger = LogService::logger('ghn_webhook', [
    ...
    {
        $orderCode = Arr::get($this->input, 'OrderCode');
        ...
        if (!$order = $this->findOrder($orderCode)) {
            ...
            return null;
//            throw new ShippingPartnerException("GHN.WEBHOOK.ORDER_NOT_FOUND: {$orderCode}");
        }

        $order->update(['original_status' => $status]);

        if (!$orderStatus = Service::ghn()->mapStatus($status)) {
            $this->logger->error('WEBHOOK.CANT_MAP_STATUS');
            throw new ShippingPartnerException("GHN.WEBHOOK.CANT_MAP_STATUS: " . json_encode([
                'order' => $orderCode,
                'ghn_status' => $status,
            ]));
        }
        ...
        if (!$order->canChangeStatus($orderStatus)) {
            ...
            return null;
//            throw new ShippingPartnerException("GHN.WEBHOOK.CANT_CHANGE_STATUS: " . json_encode($errorPayload));
        }
        ...
        try {
            ...
            throw new ShippingPartnerException("GHN.WEBHOOK.CHANGE_STATUS_FAILED: " . json_encode($errorPayload));
        }
    }
    ...
    protected function findOrder($code)
    {
        return Order::query()->where([
            'shipping_partner_code' => ShippingPartner::PARTNER_GHN,
            'code' => $code,
        ])->first();
    }
}
