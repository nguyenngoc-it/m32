...
namespace Modules\JNTT\Commands;
...
class SyncOrderStatus
{
    ...
    public function __construct(array $input, User $user)
    {
        $this->input  = $input;
        $this->user   = $user;
        $this->logger = LogService::logger('jntt_webhook', [
    ...
    public function handle()
    {
        ...
        if (!$orderStatus = Service::jntt()->mapStatus($status)) {
            $this->logger->error('WEBHOOK.CANT_MAP_STATUS');
            throw new ShippingPartnerException("JNTT.WEBHOOK.CANT_MAP_STATUS: " . json_encode([
                    'order' => $orderCode,
        ...
        try {
            ...
            throw new ShippingPartnerException("JNTT.WEBHOOK.CHANGE_STATUS_FAILED: " . json_encode($errorPayload));
        }
    }
    ...
    {
        $query = Order::query()->where('shipping_partner_code', ShippingPartner::PARTNER_JNTT);
        if(!empty($code)) {
